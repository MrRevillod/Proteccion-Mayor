...
export const validateUserId =
	(role: UserRole) => async (req: Request, res: Response, next: NextFunction) => {
...
export const validateSchema = (
	schema: SomeZodObject | ZodEffects<ZodObject<any, any>> | AnyZodObject | any,
) => {
...
export const eventOwnerValidation = async (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		if (
			!event ||
			(event.professionalId !== responseUserId && event.seniorId !== responseUserId)
		) {
			throw new AppError(401, "No tienes permisos para acceder a este recurso")
		}
		next()
