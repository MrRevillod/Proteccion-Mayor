...
export class EventController {
	...
	/**
	 * Controlador para obtener una lista de Eventos
	...
	public getMany: Controller = async (req, res, handleError) => {
		try {
			const query = EventSchemas.query.parse(req.query)
			const data = await prisma.event.findMany({
				select: EventSchemas.defaultSelect,
				where: {
			...
			return res
				.status(200)
				.json({ values: { formatted: events.formatted, byId: events.byId } })
		} catch (error) {
			handleError(error)
		}
	}
	...
	public deleteOne: Controller = async (req, res, handleError) => {
		...
		try {
			...
			const { formatted } = this.service.format([event])

			io.to("ADMIN").emit("event:delete", formatted.at(0))
			io.to(event.professionalId as string).emit("event:delete", formatted.at(0))

			return res.status(200).json({ values: { modified: formatted.at(0) } })
		} catch (error) {
			handleError(error)
		}
	}
	...
	public createReservation: Controller = async (req, res, handleError) => {
		...
		try {
			...
			const { professionalTemplate, seniorTemplate } =
				generateReservationTemplates(updatedEvent)
	...
	public cancelReservation: Controller = async (req, res, handleError) => {
		...
		try {
			...
			const { professionalTemplate, seniorTemplate } = generateCancelTemplates(updated)
			...
			const { formatted } = this.service.format([updated])

			io.to("ADMIN").emit("event:update", formatted.at(0))
			io.to(event.professionalId as string).emit("event:update", formatted.at(0))

			return res.status(200).json({ modified: formatted.at(0), values: {} })
		} catch (error) {
			handleError(error)
		}
	}
