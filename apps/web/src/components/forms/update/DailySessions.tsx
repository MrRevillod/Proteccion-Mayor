		...
		<Modal type="Other" loading={loading} title={`Número de atenciones diarias ${selectedData?.name}`}>
			<FormProvider {...methods}>
				<Form actionType="update" refetch={refetch} setLoading={setLoading} action={updateDailySessions}>
					<p>
						<strong>Nota:</strong> El número de atenciones diarias está relacionado a la cantidad de
						atenciones que realizará un profesional del servicio seleccionado en el centro de atención.
					</p>
					{dailySessions.map((dailySession) => (
						<div key={dailySession.id} className="flex items-center w-full bg-blue-800">
							<h3 className="text-sm font-semibold w-3/4">{`Servicio de ${dailySession.service?.name}`}</h3>
							<div className="w-1/4">
								<Input
									type="number"
									label=""
