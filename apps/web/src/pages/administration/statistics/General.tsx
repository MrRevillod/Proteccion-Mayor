...
const GeneralStatisticsPage: React.FC = () => {
    ...
    const [serviceState, setServiceState] = useState<{ series: ApexAxisChartSeries, options: ApexOptions }>(barChartState)
    ...
    useEffect(() => {
        setTableProfessional(Object.keys(proffesionalState).map((key) => {
            return {
                id: key,
                ...
                total: proffesionalState[key].assistance + proffesionalState[key].absence + proffesionalState[key].unreserved
            }
        }))
    ...
    useEffect(() => {
        ...
        setState({
            ...
            series: [
                { name: "Ausencias", data: reportData?.absence }, { name: "Asistencias", data: reportData?.assistance }, {
                    name: "Sin reservar", data: reportData?.unreserved
                }
            ],
            options: { ...state.options, xaxis: { ...state.options.xaxis, type: 'datetime', min: new Date(from).getTime(), } }
        })
        ...
        setCenterState({
            ...
            options: {
                ...centerState.options, xaxis: { ...centerState.options.xaxis, categories: reportData ? Object.keys(reportData?.splitted.center) : [] }
            }
        })
        ...
        setServiceState({
            ...
            options: {
                ...serviceState.options, xaxis: { ...serviceState.options.xaxis, categories: reportData ? Object.keys(reportData?.splitted.service) : [] }
            }
        })
                    ...
                    <div className={"grid grid-cols-4 gap-7"}>
                        <div className="bg-white dark:bg-primary-darker  p-4 rounded-md">
                            <Descriptions title=""   bordered column={1} size="small">
                                ...
                                <Descriptions.Item label="Asistencias totales">{sumColumn(reportData?.assistance || [[0, 0]], 1)}</Descriptions.Item>
                                <Descriptions.Item label="Ausencias totales">{sumColumn(reportData?.absence || [[0, 0]], 1)}</Descriptions.Item>
                                <Descriptions.Item label="Sin reservar">{sumColumn(reportData?.unreserved || [[0, 0]], 1)}</Descriptions.Item>
