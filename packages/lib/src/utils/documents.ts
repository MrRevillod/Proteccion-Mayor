...
export const generarExcel = async (datos: EventPrismaResult[]): Promise<Buffer> => {
    ...
    datos.forEach((dato) => {
      worksheet.addRow({
        ...
        centro: dato.center?.name || "Sin centro", // Convierte el ID del centro a un nombre
        profesional: dato.professional?.name || 'Sin profesional',
        servicio: dato.service?.name || "Sin servicio", // Convierte el ID del servicio a un nombre
        personaMayor: dato.senior?.name || 'Sin persona mayor',
